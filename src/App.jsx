...
function SurahGrid({ surahNumber, navigate }) {
  ...
  return (
    <div style={styles.gridContainer}>
      ...
      <button style={styles.backButton} onClick={() => navigate("/")} className="back-button">
      ...
      <div style={styles.gridScrollContainer}>
        <div style={styles.ayahGrid}>
          {verses.map((verse) => (
            <div
              key={verse.id}
              style={styles.gridCard}
              className="grid-card"
              onClick={() => navigate(`/surah/${surahNumber}/${verse.ayahNumber}`)}
            >
              <div style={styles.gridCardNumber}>{verse.ayahNumber}</div>
              <p style={styles.gridCardTextArabic}>{verse.textArabic.substring(0, 50)}...</p>
              <p style={styles.gridCardTextEnglish}>{verse.textEnglish.substring(0, 60)}...</p>
            </div>
          ))}
        </div>
      </div>
...
function SurahPage({ surahNumber, startAyah, navigate, readingStats, setReadingStats }) {
  const [verses, setVerses] = useState([])
  ...
  // const [readingStats, setReadingStats] = useState({}) // REMOVED: Now managed at App level
  ...
  useEffect(() => {
    loadSurahVerses()
    loadLikedStatus()
    // loadReadingStats() // REMOVED: Now managed at App level
  }, [surahNumber])

  // const loadReadingStats = async () => { // REMOVED: Now managed at App level
  //   try {
  ...
  //     }
  //   } catch (err) {
  //     console.log("Error loading reading stats:", err)
  //   }
  // }

  useEffect(() => {
    if (verses.length > 0 && startAyah && containerRef.current) {
      const ayahIndex = verses.findIndex((v) => v.ayahNumber === Number.parseInt(startAyah))
      if (ayahIndex !== -1) {
        setTimeout(() => {
          containerRef.current.scrollTop = ayahIndex * containerRef.current.clientHeight
          setCurrentIndex(ayahIndex)
        }, 100)
      }
    }
  }, [verses, startAyah])
  ...
  const trackAyahView = async (verse) => {
    ...
    try {
      ...
      const ayahsArray = Array.isArray(stats[surahKey].ayahsRead) ? stats[surahKey].ayahsRead : []
  ...
  const toggleLike = async (verse) => {
    ...
    try {
      if (isLiked) {
        ...
        const result = await storage.get("liked-ayah-ids")
        const likedIds = result && result.value ? JSON.parse(result.value) : []
  ...
  const likedVerses = verses.filter((verse) => liked[verse.id])
  ...
  return (
    <div style={styles.container}>
      ...
      <div ref={containerRef} style={styles.scrollContainer} onScroll={handleScroll}>
        {verses.map((verse) => (
          <div key={verse.id} style={styles.verseSlide} onClick={() => handleVerseClick(verse)}>
            <div style={styles.verseContent}>
              ...
              <div style={styles.sidebar} onClick={(e) => e.stopPropagation()}>
                ...
                >
                  ...
                  />
                  <span style={styles.iconLabel}>{liked[verse.id] ? "Liked" : "Like"}</span>
                </button>
...
export default function App() {
  ...
  const loadReadingStats = async () => {
    try {
      const result = await (window.storage ? window.storage.get("reading-stats") : null)
      const value = result?.value || localStorage.getItem("reading-stats")
      if (value) {
...
// Main Home Component
function HomePage({ navigate, readingStats, setReadingStats }) {
  const [verses, setVerses] = useState([])
  const [currentIndex, setCurrentIndex] = useState(0)
  ...
  const trackAyahView = async (verse) => {
    ...
    try {
      ...
      await storage.set("reading-stats", JSON.stringify(stats))
      setReadingStats(stats) // Update global state
    } catch (err) {
 ...
 const fetchRandomVerse = async () => {
  ...
  return (
    <div style={styles.container}>
      ...
      <div ref={containerRef} style={styles.scrollContainer} onScroll={handleScroll}>
        {verses.map((verse) => (
          <div key={verse.id} style={styles.verseSlide} onClick={() => navigate(`/surah/${verse.surahNumber}`)}>
            <div style={styles.verseContent}>
              <div style={styles.textContainer}>
                <p style={styles.surahName}>{verse.surahNameArabic}</p>
                <p style={styles.verseTextArabic}>{verse.textArabic}</p>
